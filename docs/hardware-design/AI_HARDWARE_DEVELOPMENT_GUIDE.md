...
The status worker reads SOC immediately and then every 30 seconds, outside the LVGL lock. It updates a bottom-right battery icon and percentage under the lock: 0–20% is red, 21–100% uses muted text, and invalid SOC clears the previous percentage and displays valid measured voltage (for example `3.94V`) with a neutral battery outline. Only when both readings are unavailable does it show `--%`. Later valid SOC restores the percentage. Voltage is not converted to an estimated SOC. Refreshes do not reset inactivity timers or wake the backlight. The BSP exposes no charging-state API, so the UI does not indicate charging.

...
- 在启用自动轻睡眠时保持原生 USB Serial/JTAG 串口可用：每 2 秒轮询总线状态并带 5 秒释放宽限；主机保持总线活跃期间，固件同时持有禁止轻睡眠锁和 80 MHz APB 频率下限锁，因此主机侧的短暂总线扰动（例如其他程序探测串口）不会再释放串口锁并导致端口永久失效。总线持续静默或拔线 5 秒后仍允许电池轻睡眠；仅在真实主机挂起后丢失的串口需要重新插拔恢复。

- 语音任务中的音频关闭失败改为重试或上报设备错误并回到闲置，不再中止固件。

- 降低熄屏待机活动：闲置时关闭 ES8311/I2S，录音任务等待事件，熄屏暂停 LVGL tick/刷新/动画，并启用动态调频、自动轻睡眠及 BLE Modem-sleep。ADC 按键改为 20 ms 扫描，相同主机状态不再重置亮屏超时。实际节电幅度、唤醒和音频表现仍须真机测量。

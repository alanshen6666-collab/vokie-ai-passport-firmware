...
The audio worker owns stream open/read/write/suspend. After the boot sound and each recording, `bsp_audio_suspend()` closes ES8311 and both I2S channels; the worker blocks on a task notification until capture is requested. Reopening restores the same 16 kHz format and starts fresh DMA. A no-light-sleep lock protects the entire open-stream interval, including boot playback. Failed opens clean up before a later retry. The amplifier enable is not wired to the MCU, so software cannot eliminate its static current.
...
USB Serial/JTAG connection monitoring holds a sleep lock while USB is connected (`CONFIG_USJ_NO_AUTO_LS_ON_CONNECTION=y`), because the ESP32-C3 USB peripheral cannot survive light sleep. Unplug USB to exercise battery standby; USB logging is not a valid light-sleep current measurement.

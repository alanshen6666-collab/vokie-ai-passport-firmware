...
- **复用既有组件**：可复用硬件逻辑放 `components/bsp`；Vokie 产品行为、BLE 传输与状态界面放 `main`。不要另造轮子，遵循既有 BSP API。
- **保持协议文档同步**：修改 Vokie 应用时，必须同步核对 BLE V1 线协议、主机生命周期、按键语义和状态变化。
...
- **关注功耗**：本设备是小电池供电的可穿戴产品，编写固件时要留意功耗。当前 Vokie 固件会在空闲时调暗或关闭 LCD 背光，但没有 light/deep sleep 应用模式；不要把背光控制描述成 MCU 进入睡眠。相关指引见 [`../../hardware-design/AI_HARDWARE_DEVELOPMENT_GUIDE.zh_CN.md`](../../hardware-design/AI_HARDWARE_DEVELOPMENT_GUIDE.zh_CN.md)。

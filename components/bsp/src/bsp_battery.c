...
// CW2017 电量计驱动。
// Profile and upload sequence: FoloToy/ai-passport PR #38, commit
// 7eeb76dbdf4038cda70817a1852b7737cdad36bc (MIT), for the stock 520 mAh cell.
...
// 首次计算期间 SOC 可能大于 100；每 100 ms 检查一次，最多 50 次。
// 总等待包含 5 秒延时及各次 I2C 事务耗时，始终在启动任务上下文执行。
...
esp_err_t bsp_battery_init(void) {
    ...
    uint8_t mode = 0xff;
    if (cw_read(CW_REG_CONFIG, &mode, 1) != 0 || mode != CW_CONFIG_ACTIVE) {
...
int bsp_battery_mv(void) {
    ...
    int mv = (int)((raw * 3125) / 10000);                   // raw * 312.5uV → mV
    // Reject reset/invalid ADC data outside the CW2017 measurement range.
    return mv >= 2500 && mv <= 4900 ? mv : -1;
}

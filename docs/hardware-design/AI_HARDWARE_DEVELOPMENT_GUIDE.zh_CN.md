...
状态工作任务立即读取一次 SOC，之后每 30 秒读取一次，I2C 读取在 LVGL 锁外执行。持锁更新右下角的电池图标和百分比：0–20% 标红，21–100% 使用灰色文字，SOC 无效时清除旧百分比，若实测电压有效则显示电压（例如 `3.94V`）和中性电池轮廓；只有两种读数都不可用时才显示 `--%`。之后 SOC 有效时恢复百分比，不将电压换算为估计电量。刷新不会重置空闲计时或唤醒背光。BSP 未提供充电状态 API，因此界面不显示充电状态。

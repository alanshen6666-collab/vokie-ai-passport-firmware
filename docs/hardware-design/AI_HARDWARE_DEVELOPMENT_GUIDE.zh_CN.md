...
| 电池 | CW2017 电量计 | 共享 I2C0，地址 0x63 | 状态界面显示电量百分比，SOC 无效时显示实测电压 |
| Wi-Fi | 当前固件不使用 | 没有 Wi-Fi 应用链路 | 未编译进 Vokie 应用 |
| Bluetooth LE | NimBLE 外设 | Vokie BLE V1 服务 | 可连接广播与 Notification |
| 低功耗 | 事件驱动闲置与自动轻睡眠 | ESP-IDF PM + BLE Modem-sleep | 音频/背光睡眠锁；ADC 三键每 20 ms 扫描；无深睡眠 |
...
显示/LVGL 是状态界面的硬依赖。音频和按键由 `vokie_ble_start()` 初始化；初始化失败会记录错误，设备无法提供语音输入。应用在启动 UI 和音频工作任务前通过 BSP 初始化可选的 CW2017；初始化失败不会阻止启动。公开 BSP API 位于 `components/bsp/include/`：
...
音频任务独占流的打开、读写、暂停和关闭。开机音及每次录音结束后，`bsp_audio_pause()` 先静音并关闭扬声器 DAC，再停止两个 I2S 通道并释放音频禁止轻睡眠锁，同时保留 ES8311 麦克风偏置及滤波器状态。音量为零不等于硬件静音。由于当前 codec 包装层可能吞掉 I2C 写错误，BSP 会读回 DAC 静音与供电寄存器；转换失败时为仍运行的通道保留锁以便重试。打开或恢复录音时保持输出关闭，仅在 I2S 运行且设置非零播放音量时开启。任务阻塞等待通知。同格式恢复时启动新的 DMA，不关闭重开 codec，首次 16-bit 单声道读取先读走一个不完整的 I2S 槽采样（16 kHz 下为 62.5 微秒），再返回完整请求帧，从而避免完全关闭 codec 后实测出现的重复 ADC 启动瞬态。`bsp_audio_suspend()` 保留为彻底关闭及格式切换接口。转换失败时，只要仍有活动通道就保留睡眠锁并允许重试。codec 模拟供电及使能未连接 MCU 的功放仍贡献静态电流；净节电幅度及电池轻睡眠后的恢复仍须测量。

UI 任务按事件通知和准确到期时间运行，替代 200 ms 轮询。相同的主机状态/消息不重置屏幕超时。熄屏暂停 LVGL 刷新、动画和 5 ms port tick；唤醒补回 LVGL 流逝时间。暂停各定时器时保持定时器处理器启用，避免 LVGL 9.5 在处理器禁用时每 1 ms 空转。port 任务仍可能按最长 500 ms 间隔唤醒。LCD 控制器不下电。

电源管理允许 40–160 MHz 动态调频和 tickless idle 自动轻睡眠。BLE Modem-sleep 使用主晶振，并开启轻睡眠期间主晶振供电选项，不假定存在外部 32 kHz 晶振。背光使用 XTAL 时钟，亮屏时持有禁止轻睡眠锁，避免 PWM 闪烁。ADC 三键保留每 20 ms 扫描，去抖为一个扫描周期；按键时序和快速短按仍需真机验证。BLE 连接参数不变。

USB Serial/JTAG 待机（`main/usb_standby.c`）每 2 秒轮询总线状态；主机保持总线活跃期间同时持有禁止轻睡眠锁和 80 MHz APB 频率锁（`CONFIG_USJ_NO_AUTO_LS_ON_CONNECTION=y` 仍作为驱动内快速路径保留）。APB 下限在动态调频降至 40 MHz 时保持 BBPLL 供电，从而保住 USB PHY 时钟；5 秒释放宽限在短暂 SOF 间隙期间保留锁。此前孤立断连的原因尚未确认。总线持续静默或拔线 5 秒后重新允许电池轻睡眠。拔掉 USB 才能测试电池待机；连接 USB 读日志不能作为轻睡眠电流测量。主机挂起及重插行为须独立完成硬件验收。

在断开 USB/调试连接后，测量未连接熄屏、已连接熄屏、亮屏空闲及录音的电池侧电流。验证首字录音、开机音打断、至少 20 轮闲置/录音循环、三键和长按、断连重连、亮度超时不变，以及闲置后 PM 锁不残留。构建或主机测试不能证明实际节电幅度。

## 9. CW2017 电池计

初始化使用 [FoloToy PR #38](https://github.com/FoloToy/ai-passport/pull/38) 提供的原装 520 mAh 电池 profile，固定到[提交 `7eeb76db`](https://github.com/FoloToy/ai-passport/blob/7eeb76dbdf4038cda70817a1852b7737cdad36bc/components/bsp/src/bsp_battery.c)。先读取 VERSION，同时比较 UPDATE_FLAG 和全部 80 字节 profile；标志缺失或内容不匹配时，按照上游顺序进入休眠，逐字节写入、完整读回校验，保留告警阈值并设置 UPDATE_FLAG，然后激活。已匹配的 profile 不重复写入。确认正常模式后，以 100 ms 间隔等待首次有效 SOC，最多 50 次，另加 I2C 事务耗时。失败时释放设备句柄，允许之后重试。

应在 UI/音频工作任务启动前串行初始化。初始化和读取均可能阻塞，不得放在按键回调或 LVGL 任务中。该 profile 仅针对量产原装 520 mAh 电池；更换电芯后需要匹配的厂商 profile 和重新验证充放电曲线。此操作只更新 CW2017 的 profile 寄存器，不修改 ESP32 的 NVS、身份区、分区布局或 Recovery。

- SOC：读 0x04–0x05，仅返回高字节整数百分比；大于 100 视为未就绪并返回 `-1`。
- 电压：读 0x02–0x03 的 14 bit 值，换算为 `raw × 312.5 µV`，API 返回 mV。超出手册规定的 2500–4900 mV 测量范围时返回 `-1`。
- 事务超时当前为 100 ms，设备时钟为 100 kHz。
- 芯片不应答时初始化返回 `ESP_ERR_NOT_FOUND`；状态界面显示 `--%` 并继续运行。初始化时缺失的电量计会在下次开机时重新检测。

状态工作任务立即读取一次 SOC，之后每 30 秒读取一次，I2C 读取在 LVGL 锁外执行。亮屏时持锁更新屏幕右上角的电池图标和百分比。熄屏时仅缓存采样，不更新 LVGL，唤醒时展示最新采样。电量在亮屏时持续可见，独立于 VOICE/SEND/UNDO 提示层。开机时展示全部三项提示与引导线，三秒后淡出，即使主机进入活动状态也不延长。UP 选择 VOICE，DOWN 选择 SEND，OK 选择 UNDO，仅展示选中文案、对应引导线及单行背景。单项提示统一放在原来 SEND 所在的中间一行，指引线从这里出发，保留对应实体按键的终点；全部提示模式保留原来的三行布局与指引线路径。换键时立即替换上一项。由按键唤出的提示在录音或处理中保持显示，最后一次按键空闲满 3 秒后淡出。独立硬件电源键没有应用层按下回调，不能用于在运行中唤出提示。主机状态变化不会唤出隐藏的按键提示。后台电量刷新也不会唤出提示、重置空闲计时或唤醒背光；屏幕变暗和熄屏策略保持不变。0–20% 标红，21–100% 的图标和文字使用低调灰色（`#646C78`）。SOC 无效时清除旧百分比，若实测电压有效则显示电压（例如 `3.94V`）和中性电池轮廓；两种读数都不可用时显示 `--%`，SOC 恢复有效后重新显示百分比，不将电压换算为估计电量。BSP 未提供充电状态 API，因此界面不显示充电状态。

I2C 应答和合理电压不能保证 SOC 有效：profile 为空时，SOC 可能一直超出范围，直至写入与电芯匹配的参数。首次写入后读数仍可能处于稳定过程，或暂时降级显示电压。完整充放电准确度应与初始化和界面行为分开验证。
...
| 电池 | 合理 SOC/电压、0/20/21/100%、电压降级与未知读数显示、读取失败恢复、无电量计时启动、与按键提示无重叠、背光超时不变、录音并发稳定 |

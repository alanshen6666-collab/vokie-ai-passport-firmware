...
音频任务独占流的打开、读写和暂停。开机音及每次录音结束后，`bsp_audio_suspend()` 关闭 ES8311 和两个 I2S 通道；任务阻塞等待录音通知。重开恢复相同 16 kHz 格式并启动新的 DMA。禁止轻睡眠锁保护整个流打开期间，包括开机音。打开失败后清理，允许后续重试。功放使能未连接 MCU，软件无法消除其静态电流。
